...
// SetHighlighter sets a function that will be used to apply syntax highlighting
// to the input text. The function should receive the entire input text and return
// a highlighted version. Pass nil to disable highlighting.
...
func (m Model) View() string {
	...
	// Use original value for structure and cursor positioning
	highlightedValue := m.value

	// Get highlighted text if available (as complete lines, not runes)
	var highlightedLines []string
	if m.Highlighter != nil {
		highlightedText := m.Highlighter(m.Value())
...
// insertCursorInHighlightedText replaces the character at cursorPos with the cursor in highlighted text
...
func findPositionInHighlightedText(highlightedText string, charPos int) int {
	if charPos <= 0 {
		return 0
	}
	...
	for bytePos < len(highlightedText) {
		if highlightedText[bytePos] == '\x1b' {
			inEscape = true
		} else if inEscape && highlightedText[bytePos] == 'm' {
			inEscape = false
			bytePos++
			continue
		}
		...
		if !inEscape {
			...
			// Move to next character (handle UTF-8)
			if highlightedText[bytePos] < 0x80 {
				plainCharCount++
			} else if (highlightedText[bytePos] & 0xE0) == 0xC0 {
				// 2-byte UTF-8
				plainCharCount++
				bytePos++
			} else if (highlightedText[bytePos] & 0xF0) == 0xE0 {
				// 3-byte UTF-8
				plainCharCount++
				bytePos += 2
			} else if (highlightedText[bytePos] & 0xF8) == 0xF0 {
				// 4-byte UTF-8
				plainCharCount++
				bytePos += 3
			} else {
